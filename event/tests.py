...
class LoginTest(test.LiveServerTestCase):
    ...
    def test_login(self):
        ...
        for usr, pwd in zip(usernames, passwords):
            ...
            username = self.selenium.find_element("id", "id_username")
            password = self.selenium.find_element("id", "id_password")
            submit = self.selenium.find_element("id", "id_submit")
            ...
            try:
                profile = self.selenium.find_element("id", "id_profile")
                self.assertEqual("Profile", profile.text, "Login successful")
